...
1. **Initialization**
	...
	3. **Initialize a stack** and **push** the starting node onto the stack.
2. **Exploration**
	...
	2. If the current node **has not been visited:**
		...
		2. **Push** all unvisited adjacent nodes onto the stack.
...
아래와 같은 그래프에서 Iterative DFS 방법은 동일한 노드가 두 번 이상 스택에 쌓이는 경우가 발생할 수 있다. Recursive DFS는 알고리즘 구조상 두 번 이상 스택에 쌓이는 경우는 없다. Iterative DFS를 개선할 수 있는 방법이 있을까?
